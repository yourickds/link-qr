...
2. Изменяет права владения и доступ для директорий `models`, `controllers` и `views`. Это необходимо для корректной работы Gii (генератор компонентов Yii) во время разработки.:
   ...
   - Права доступа: `775`
3. Изменяет права владения и доступ для директорий `web/assets`. Это нужно для генерации assets файлов
4. На всякий случай проверяет доступность mysql порта через nc
5. Выполняет установки зависимостей и выполняет миграции
